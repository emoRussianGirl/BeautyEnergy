...
for (let i = 0; i < containers.length; i++){
    sliders[i] = {
        ...
        itemWidth: (containers[i].clientWidth) / 5,
        movePosition: 3 * ((containers[i].clientWidth) / 5),
        setPosition(){
    ...
    sliders[i].btnNext.addEventListener('click', () => {
        ...
        sliders[i].position -= itemsLeft >= sliders[i].slidesToScroll? 
            sliders[i].movePosition : 
            itemsLeft * sliders[i].itemWidth;
